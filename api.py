...
# get username based on ID used in msgapp
@get('/<userID>/name')
def get_user_name(userID,db):
    user_name = query(db, 'SELECT userName  FROM users WHERE  userID = ?', [userID])
    if not user_name:
        abort(404,"get user name with ID error")
    return {'name': user_name}

#get ID based on username used in msgapp.py
@get('/<usrName>/ID')
def get_user_id(usrName,db):
    usrID = query(db, 'SELECT userID  FROM users WHERE  userName = ?', [usrName])
    if not usrID:
        abort(404,"get user name with ID error")
    return {'ID': usrID}
